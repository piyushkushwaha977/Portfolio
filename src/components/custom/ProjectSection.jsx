  ...
  <motion.div
    ...
    <div className='mb-24 mx-2 md:mx-8  mt-8  md:mt-16 md:max-h-[36rem] md:max-w-[60rem] lg:max-h-[44rem] lg:max-w-[84rem] rounded-lg md:rounded-[25px] flex flex-col-reverse lg:flex-row lg:mx-auto shadow-2xl shadow-gray-900  '>
      <div className='rounded-lg  md:rounded-[20px] flex flex-col gap-2  md:gap-6 p-4 lg:p-0 justify-center  lg:w-[35%] text-white '>
        ...
        <div className=' px-4 md:px-12 text-center text-sm md:text-[15px] sm:mx-4  text-neutral-400 mx-auto font-poppins'>
          {description}
        </div>

        <div className=' flex justify-center items-center md:w-8/12 mx-auto px-2  '>
           <div className=' w-[4px] h-24 md:h-20 lg:h-28 rounded-lg bg-gradient-to-b from-neutral-700 via-neutral-600 to-neutral-800'></div>
                <div
                     initial={{opacity: 0 , x: -100}}
                     whileInView={{opacity:1 , x: 0}}
                     ...
                     className=" w-full my-2 mx-2 flex gap-1 md:gap-2 flex-wrap">
                      ...
                      {TechList.map( (name,index) => (
                        <div            
                         key={index}
                        className=" w-fit cursor-pointer text-sm  text-neutral-400 font-semibold px-2 md:px-2 py-0.5 rounded-lg
                         border border-gray-600 transition-all duration-200 hover:text-white hover:scale-105 "
                        >{name}</div>
                      ))}
